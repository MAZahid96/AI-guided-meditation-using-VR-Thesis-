...
public class Scene2Controller : MonoBehaviour
{
    ...
    IEnumerator MeditationRoutine()
    {
        ...
        bool isHigh = (stress == OpenAIManager.StressLevel.High);
        // if stress is Medium or Unknown, default to Low
        if (stress == OpenAIManager.StressLevel.Medium || stress == OpenAIManager.StressLevel.Unknown)
            isHigh = false;

        // --- Fixed total duration: 4 minutes for both modes ---
        float minutes = 4f;
        float meditationTime = minutes * 60f;

        // --- Intro line tailored to mode ---
        string intro = isHigh
            ? "We will keep a steady pace with frequent guidance and longer exhales."
            : "We will move slowly with spacious pauses and gentle breathing.";
        ...
        // --- Gaps per mode (seconds between each spoken statement) ---
        float gapSeconds = isHigh ? 15f : 35f;
